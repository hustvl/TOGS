...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background,scene):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        times1 = scene.getTraintime().copy()
        times2 = scene.getTesttime().copy()
        if view.image_name in times1.keys():
            view_time = times1[view.image_name]
        else:
            view_time = times2[view.image_name]

        rendering = render(view, gaussians, pipeline, background,view_time = view_time)["render"]
